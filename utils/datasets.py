...
def detect_contour(path):
    ...
    cntr = plt.contour(pos_x, pos_y, img, 8)
...
def resize(image, size):
    ...
    #else:
    #    image = F.interpolate(image.unsqueeze(0), size=(size[0],size[1]), mode="nearest").squeeze(0)
    return image
...
class ListDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print(img_path)
        # Extract image as PyTorch tensor
        #img_org = transforms.ToTensor()(Image.open(img_path))
        ...
        #contour_maps = detect_contour(img_path)
        ...
        #return img, targets, contour_maps
